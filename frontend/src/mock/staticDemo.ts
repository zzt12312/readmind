...
const noteFilters = {
  categories: ['心理学', '商业', '历史', '经济'],
  tags: ['长期主义', '制度', '复利', '组织', '注意力', '自我接纳', '判断力', '危机'],
  chapters: ['第一章 大脑', '第三章 行动力', '财富', '幸福', '弘光政权', '隆武政权', '地方政府', '财政与激励', '课题分离'],
}
...
const graphClusters = [
  {
    ...
    book_count: 2,
    sample_books: [
      { id: 1, title: '认知觉醒', cover: '', notes: 42 },
      { id: 2, title: '纳瓦尔宝典', cover: '', notes: 36 },
    ],
    sample_excerpts: [
  ...
  },
  {
    ...
    book_count: 2,
    sample_books: [
      { id: 3, title: '南明史', cover: '', notes: 83 },
      { id: 4, title: '置身事内', cover: '', notes: 51 },
    ],
    sample_excerpts: [
  ...
  },
  {
    id: 2,
    name: '自我与关系',
    topics: ['自由', '幸福', '课题分离', '自我接纳'],
    note_count: 3,
    book_count: 2,
    sample_books: [
      { id: 5, title: '被讨厌的勇气', cover: '', notes: 29 },
      { id: 2, title: '纳瓦尔宝典', cover: '', notes: 36 },
    ],
    sample_excerpts: [
...
const reviewCards = demoNotes.slice(0, 6).map((note, index) => ({
  id: note.id,
...
function buildDashboard() {
  return {
    ...
    active_topics: ['长期主义', '制度', '复利', '组织', '自我接纳', '注意力', '判断力'],
    review_summary: {
      suggested_count: 6,
      due_count: 6,
      streak_days: 2,
    ...
    recommended_review: {
      ...
      path: '/graph',
      topics: ['制度', '组织', '激励'],
      book: { id: 3, title: '南明史', author: '顾诚', notes: 83, cover: '' },
    },
  }
}
...
function buildAnalytics() {
  return {
    metrics: [
      { label: '累计阅读时长', value: '52h', hint: '来自演示书籍 readingTime 缓存' },
      { label: '高价值书籍', value: 3, hint: '笔记密度和复习价值较高' },
      { label: '偏好方向', value: '历史/心理', hint: '主题雷达中最突出的两个方向' },
      { label: '复习连续', value: '2 天', hint: '演示复习进度' },
    ],
    category_preferences: [
      { category: '历史', book_count: 1, note_count: 83, share: 35 },
      { category: '心理学', book_count: 2, note_count: 71, share: 30 },
      { category: '经济', book_count: 1, note_count: 51, share: 21 },
      { category: '商业', book_count: 1, note_count: 36, share: 14 },
    ],
    preference_radar: [
    ...
    reading_time_rank: demoBooks.map((book) => ({
      id: book.id,
      title: book.title,
      author: book.author,
      ...
      reading_time_minutes: book.id === 3 ? 1116 : book.id === 4 ? 668 : book.id === 1 ? 558 : book.id === 2 ? 462 : 326,
      cover: book.cover,
    })),
    high_value_matrix: demoBooks.map((book, index) => ({
      ...
      x: 35 + index * 12,
      y: 42 + (index % 3) * 16,
      value_score: 70 + index * 4,
    })),
    topic_rank: [
      { topic: '制度', count: 12, share: 18 },
      { topic: '长期主义', count: 10, share: 15 },
      { topic: '组织', count: 9, share: 13 },
      { topic: '复利', count: 8, share: 12 },
      { topic: '自我接纳', count: 6, share: 9 },
    ],
    review_funnel: [
    ...
    reading_timeline: [
      ...
      { period: '2026-02', book_count: 1, books: ['纳瓦尔宝典'] },
      { period: '2026-03', book_count: 2, books: ['认知觉醒', '置身事内'] },
      { period: '2026-04', book_count: 1, books: ['被讨厌的勇气'] },
    ],
    author_cloud: [
      { author: '顾诚', book_count: 1, note_count: 83, weight: 90 },
      { author: '周岭', book_count: 1, note_count: 42, weight: 68 },
      { author: '兰小欢', book_count: 1, note_count: 51, weight: 72 },
      { author: '纳瓦尔', book_count: 1, note_count: 36, weight: 58 },
    ],
...
function buildGraph() {
  return {
    overview: { topic_count: 12, cluster_count: 3, edge_count: 6, book_count: demoBooks.length },
    filters: {
